...
// Camp detail pages, structural cues borrowed from the Swiss-education-agency
// genre (hero photo, location/programme/activities breakdown, dates) but
// without the agency upsell blocks (no "Education Services", no "Enquire
// Now") — we're a peer forum, so the page ends by pointing at the forum
// instead of a lead-gen form.
...
export default async function CampPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const otherCamps = Object.entries(CAMPS).filter(([s]) => s !== slug);
  ...
  return (
    <>
      ...
      <Breadcrumb
        items={[
          { label: "Home", href: "/" },
          { label: "Education", href: "/education#summer-camps" },
          { label: camp.name },
        ]}
      />
      <div className="flex-1">
        ...
        <div className="mx-auto w-full max-w-4xl px-5 py-10">
          ...
          <section className="mt-14">
            <h2 className="font-display mb-4 text-[1.15rem] font-medium text-foreground">
              Other Swiss summer camps
            </h2>
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {otherCamps.map(([otherSlug, otherCamp]) => (
                <Link
                  key={otherSlug}
                  href={`/camps/${otherSlug}`}
...
function NotFound() {
  return (
    <>
      <Nav />
      <Breadcrumb items={[{ label: "Summer Camps" }]} />

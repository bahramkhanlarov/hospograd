...
// Hospitality school detail pages — same structure as /camps/[slug] and
// /boarding-schools/[slug]: hero photo, quick facts, campuses, programs,
// fees. The forum CTA links into the existing /schools/[slug] page (posts
// tagged to that school) instead of duplicating that functionality here.

export const dynamic = "force-dynamic";

export default async function HospitalitySchoolPage({ params }: { params: Promise<{ slug: string }> }) {
  const { slug } = await params;
  const school = HOSPITALITY_SCHOOLS[slug];
  if (!school) return <NotFound />;

  const otherSchools = Object.entries(HOSPITALITY_SCHOOLS).filter(([s]) => s !== slug);
    ...
    <>
      ...
      <div className="flex-1">
        ...
        <div className="mx-auto w-full max-w-4xl px-5 py-10">
          ...
          {/* Community CTA, links into the existing forum page for this school */}
          <section className="mt-10 rounded-md border border-border bg-secondary p-5">
            ...
            <p className="mb-4 text-[0.85rem] leading-relaxed text-muted-foreground">
              Read what {school.short} students are actually saying, or ask
              your own question.
            </p>
            <Link
              href={`/schools/${slug}`}
          ...
          <section className="mt-14">
            <h2 className="font-display mb-4 text-[1.15rem] font-medium text-foreground">
              Other hospitality schools
            </h2>
            <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-4">
              {otherSchools.map(([otherSlug, otherSchool]) => (
                <Link
                  key={otherSlug}
                  href={`/hospitality-schools/${otherSlug}`}
...
function NotFound() {
  return (
    <>
      <Nav />
      <Breadcrumb items={[{ label: "Hospitality Schools" }]} />
